...
class LLMInference:
    ...
    def __init__(self, model_path: str, device: Optional[str] = None):
        ...
        # Move to target device explicitly to avoid accelerate/device_map conflicts
        if device == "cuda":
    ...
    def chat(self, question: str) -> str:
        ...
        response = self.generate(
            prompt, max_length=150, temperature=0.7, top_p=0.9
        )
        return response
...
def main():
    ...
    parser.add_argument(
        "--query", type=str, default=None, help="Single query (for single mode)"
    )
    parser.add_argument(
        "--questions-file",
        type=str,
